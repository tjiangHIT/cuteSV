...
def force_calling_chrom(ivcf_path, temporary_dir, max_cluster_bias_dict, threshold_gloab_dict, gt_round, read_range, threads, multi_allele):
    ...
    for chroms in dispatch:
        ...
        if len(genotype_sv_list) == 0:
            continue
        # pool_result.append(solve_fc(chroms, genotype_sv_list, temporary_dir, max_cluster_bias_dict, threshold_gloab_dict, gt_round, multi_allele))
        fx_para = [(chroms, genotype_sv_list, temporary_dir, max_cluster_bias_dict, threshold_gloab_dict, gt_round, read_range, multi_allele)]
        # print('fx_para:')
        # print(chroms)
...
def solve_fc(chrom_list, svs_dict, temporary_dir, max_cluster_bias_dict, threshold_gloab_dict, gt_round, read_range, multi_allele):
    tots = time.time()
    ...
    for chrom in svs_dict:
        ...
        for i in range(len(svs_dict[chrom])):
            ...
            if record[7] == '<TRA>' or record[7] == '<BND>':
                ...
                seq = '<' + record[0] + '>'
            gt_list.append([record[9], record[2], assign_list[i][2], record[0], record[3],
