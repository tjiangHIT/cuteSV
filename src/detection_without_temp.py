...
def intergrate_indel(chr, evidence_read, SV_size, low_bandary, svtype, max_distance, MainCandidate, sam_path):
	...
	temp = list()
	if len(MainCandidate[svtype][chr]) == 0:
		return
...
def single_pipe(sam_path, min_length, min_mapq, max_split_parts, min_seq_size, task, min_support, max_distance):
	...
	temp_result(Chr_name, min_support, min_length, 30, max_distance, candidate, sam_path)
...
def main_ctrl(args):
	...
	for i in Task_list:
		para = [(args.input, args.min_length, args.min_mapq, args.max_split_parts, args.min_seq_size, i, args.min_support, args.max_distance)]
