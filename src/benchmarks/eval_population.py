...
def main(argv):
	...
	starttime = time.time()
	if args.handle == 'Sta':
		population_statistic(args.pop, args.output)
	if args.handle == 'Comp':
		compare_callsets(args.pop, args.base, args.output)
	logging.info("Finished in %0.2f seconds."%(time.time() - starttime))

USAGE="""\
	Evaluate population SV callsets.
	Author: Shuqi Cao
...
def parseArgs(argv):
	...
	parser.add_argument("handle", type=str, help="The aspect of evaluation, contains Sta/Comp.")
	parser.add_argument("--pop", type=str, help="Merged population callsets from force calling methods.")
	parser.add_argument("--base", type=str, help="Worldwide population callsets.")
